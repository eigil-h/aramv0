 ...
 */
aram::service::double_buffer::double_buffer(int buffer_size) : b_idx(0), bbuf_ready(true)
{
...
aram::service::double_buffer::~double_buffer()
{
	delete dbuf[1];
	delete dbuf[0];
}

aram::service::buf_t* aram::service::double_buffer::front_buffer()
{
	return dbuf[b_idx];
}

aram::service::buf_t* aram::service::double_buffer::back_buffer()
{
	return dbuf[(b_idx + 1)&1];
}

void aram::service::double_buffer::swap()
{
 ...
 */
aram::service::load_and_read_buffer::load_and_read_buffer(int siz) : double_buffer(siz)
{
}

bool aram::service::load_and_read_buffer::read_front_buffer(aram::service::sample_t* to_buf, int len)
{
	bool did_swap;
...
int aram::service::load_and_read_buffer::load_back_buffer(forward_list<istream*>& istr_list)
{
	if(!bbuf_ready)
	{
		return 0;
...
int aram::service::load_and_read_buffer::load_back_buffer_and_swap(forward_list<istream*>& istr)
{
 ...
 */
aram::service::write_and_store_buffer::write_and_store_buffer(int siz) : double_buffer(siz)
{
}

bool aram::service::write_and_store_buffer::write_front_buffer(sample_t* from_buf, int len)
{
	buf_itr_t itr;
	{
		lock_guard<mutex> guard(swap_safe);
...
int aram::service::write_and_store_buffer::swap_and_store_back_buffer(ostream& ostr)
{
 ...
 */
aram::service::single_buffer::single_buffer(istream& istr)
{
	istr.seekg(0, ios::end);
...
void aram::service::single_buffer::read(sample_t* to, int len)
{
...
void aram::service::single_buffer::reset()
{
	buf_itr = buf->begin();
}
